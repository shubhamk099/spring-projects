...
public class LoggerAspect {
    ...
    @AfterThrowing(value = "execution(* com.codeplay.service.*.*(..))", throwing = "ex")
    public void logException(JoinPoint joinPoint, Exception ex) {
        logger.log(Level.SEVERE, joinPoint.getSignature() +
                " An exception thrown with the help of" + "@AfterThrowing which happened due to : " + ex.getMessage());
    }

    @AfterReturning(value = "execution(* com.codeplay.service.*.*(..))", returning = "retVal")
